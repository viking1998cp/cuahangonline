...
import a516100.pdu.cuahangonline.adapter.DienthoaiAdapter;
...
public class DienThoaiActivity extends AppCompatActivity {
    ...
    private void LoadMoreData() {
        lvDienthoai.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                intent.putExtra("thongtinsanpham", arrayDienthoai.get(position));
    ...
    private void Getdata(int Page) {
        retrofit2.Call<List<Sanpham>> callSanpham = dataService.GetdataDienthoai(Page, idDienthoai);
        callSanpham.enqueue(new Callback<List<Sanpham>>() {
            @Override
            public void onResponse(Call<List<Sanpham>> call, Response<List<Sanpham>> response) {
                if(response.body() != null && response.body().size() > 0 ){
                    lvDienthoai.removeFooterView(footerView);
                    ArrayList<Sanpham> arrayListDienthoai = (ArrayList) response.body();
                    for(int i=0;i<arrayListDienthoai.size();i++){
                        arrayDienthoai.add(arrayListDienthoai.get(i));
                    }
    ...
    private void anhxa() {
        ...
        dienthoaiAdapter = new DienthoaiAdapter(getApplicationContext(), arrayDienthoai);
